...
public class DataUtil {
    ...
    public static double[] hzToGhz(long[] cores) {
        ...
        for (int i = 0; i < cores.length; ++i) {
            rtnArr[i] = limitPrecision(hzToGhz(cores[i]));
        }
        return rtnArr;
    }
    ...
    public static double bytesToMiB(long bytes) { return limitPrecision(1d * bytes / BYTE_MIB_DIV); }

    public static double bytesToGiB(long bytes) { return limitPrecision(1d * bytes / BYTE_GIB_DIV); }

    public static double bytesToTiB(long bytes) { return limitPrecision(1d * bytes / BYTE_TIB_DIV); }

    public static double bytesToaPiB(long bytes) { return limitPrecision(1d * bytes / BYTE_PIB_DIV); }

    public static double bytesToEiB(long bytes) { return limitPrecision(1d * bytes / BYTE_EIB_DIV); }

    public static double bytesToEiB(BigInteger bytes) {
        BigInteger[] results = bytes.divideAndRemainder(BYTE_EIB_DIV_BI);
        return limitPrecision(results[0].doubleValue() + (results[1].doubleValue() / BYTE_EIB_DIV));
    }

    public static DecimalFormat Formatter() { return df; }

    public static double limitPrecision(double d) { return Math.round(d * 100.0) / 100.0; }
